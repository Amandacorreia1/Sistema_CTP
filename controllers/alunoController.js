...
export const adicionarCondicao = async (req, res) => {
  try {
    ...
    if (condicoesNaoExistem.length === 0) {
      return res
        .status(201)
        .json({ mensagem: "Condição(s) associada(s) ao aluno com sucesso" });
    } else {
    ...
    res
      .status(500)
      .json({
        mensagem: "Erro ao adicionar a condição ao aluno",
        erro: erro.message,
      });
  }
};
...
export const cadastrarAluno = async (req, res) => {
  ...
  try {
    if (!matricula || !nome || !email || !curso) {
      return res
        .status(400)
        .json({ mensagem: "Matrícula, nome, email, curso são obrigatórios" });
    }
    ...
    for (const condicao of condicoes) {
      ...
      if (typeof condicao === "object" && condicao.id && condicao.nome) {
        ...
        if (!condicaoRecord) {
          return res
            .status(400)
            .json({ mensagem: `Condição com ID ${condicao} não encontrada` });
        }
      } else {
        return res
          .status(400)
          .json({ mensagem: "Formato de condição inválido" });
      }
    ...
    res
      .status(500)
      .json({ mensagem: "Erro ao cadastrar aluno", erro: erro.message });
  }
};
...
export const atualizarAluno = async (req, res) => {
  ...
  try {
    if (!nome || !email || !curso) {
      return res
        .status(400)
        .json({ mensagem: "Nome, email, curso são obrigatórios" });
    }
    ...
    res
      .status(500)
      .json({ mensagem: "Erro ao atualizar aluno", erro: erro.message });
  }
};
  ...
  try {
    ...
    if (!aluno) {
      ...
      if (!alunoRu || !alunoRu.mat) {
        return res
          .status(404)
          .json({ mensagem: "Aluno não encontrado na API externa" });
      }
    ...
    res
      .status(500)
      .json({ mensagem: "Erro interno ao buscar aluno", erro: erro.message });
  }
};
...
export const listarAlunos = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ mensagem: "Erro ao buscar alunos", erro: erro.message });
  }
};
